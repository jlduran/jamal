...
(
	...
	find ./* -type l | \
		...
		if [ ${je_update} -eq 0 ]; then
			...
			cp -af ${WRKDIR}/world/$link ${WRKDIR}/world/$src
...
zfs_prepare()
{
	...
	*full*|send|zfs)
		je_update=0
		ZFS_JEROOT="${TMP_ZFS_POOL_NAME}/${ZFS_JAIL_NAME}"
	...
	# to ${TMP_ZFS_POOL_NAME}/${ZFS_BOOTFS_NAME}.
	#
...
zfs_build()
{
	...
	if [ $je_update -eq 0 ]; then
		zroot=${TMP_ZFS_POOL_NAME}/${ZFS_JAIL_NAME}
	fi

	# XXX why not freebsd-version -uk or similar approach (kernel/userland versions)
	...
	if [ -d "${_OVERLAYDIR}" ]; then
		# XXX not sure if bootstrap is necessary
		bootstrap_links

		msg "[jail environment] copying in overlay directory from ${_OVERLAYDIR}"
		cp -af "${_OVERLAYDIR}/" ${WRKDIR}/world/
...
zfs_generate()
{
	...
	zpool export ${TMP_ZFS_POOL_NAME}
	zroot=
